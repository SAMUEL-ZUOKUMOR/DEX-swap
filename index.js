...
async function listAvailableTokens(){
  ...
  for (const i in tokens){
      ...
      <img class="token_list_img" src="${tokens[i].logoURI}">
        ...
        `;
      div.innerHTML = html;
      //when a token is clicked selectToken() will be called
      div.onclick = () =>{
...
async function getPrice(){
    ...
    let amount = Number(document.getElementById("from_amount").value * 10 ** currentTrade.from.demicals);
...
// Function to get a quote using /swap/v1/quote. this will pass in the user's MetaMask account to use as the takerAddress
...
function openModal(side){
    ...
    //store whether the user has selected a token on the from or side
